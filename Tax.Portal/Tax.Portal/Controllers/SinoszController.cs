...
#region List
        ...
        public virtual ActionResult ListSinoszUsersForList(GridSettings grid, string Output)
        {
            ...
            else if ("XLS" == Output)
            {
                ...
                ////a lapozást exportkor kizárjuk                
                grid.PageSize = rs0.Count();
                var rs = rs0.AsQueryable().GridPage(grid, out result).ToList();

                if (rs.Count() == 0)
                    return new EmptyResult();

                var lstgrid = new List<string[]>(rs.Count());
                lstgrid.AddRange(rs.Select(r => new[] {
                    r.Id.ToString()
                    ,r.SinoszId
                    ,r.SinoszUserName
                    ,r.BirthDate.ToString()
                ...
                return new ExcelResult(Headers, ColunmTypes, lstgrid, "SinoszTagok.xlsx", "SinoszTagok");            
            }
            else if ("PDF" == Output)
            {
                return new EmptyResult();
...
//TODO: Nem eszi meg ezt valamiért [Authorize(Roles = "SysAdmin, SinoszAdmin, SinoszUser")]
        public virtual ActionResult ListAccountingDocumentsForList(GridSettings grid, string Output)
        {
            ...
            var rs0 = db.AccountingDocument
                ...
                //.SelectMany(x => db.AccountingDocument.Where(y => y.Id == x.SinoszUser.LastAccountingDocument).DefaultIfEmpty(), (a, b) => new
                //{
                //    //keresőmezők Id = c.a.Id,
                //    StatusName = a.SinoszUser.SinoszUserStatus.StatusName ?? "",
                //    SinoszUserName = a.SinoszUser.SinoszUserName,
                //    SinoszId = a.SinoszUser.SinoszId,
                //    OrganizationName = a.SinoszUser.Organization.OrganizationName ?? "",
                //    DocumentDate = null == b ? (DateTime?)null : (DateTime?)b.DocumentDate,
                //    AccountingTypeName = null != b ? b.AccountingType.AccountingTypeName : "",
                //    BirthName = a.SinoszUser.BirthName,
                //    PostCode = a.SinoszUser.Postcode.Code + " - " + a.SinoszUser.Postcode.City ?? "",
                //    HomeAddress = a.SinoszUser.HomeAddress,
                //    RelationshipName = a.SinoszUser.Relationship.RelationshipName ?? "",
                //    BirthDate = (DateTime?)a.SinoszUser.BirthDate,//(DateTime?)a.SinoszUser.BirthDate ?? (DateTime?)null,
                //    Qualification = a.SinoszUser.Qualification,
                //    BirthPlace = a.SinoszUser.BirthPlace,
                //    MothersName = a.SinoszUser.MothersName,
                //    Telephone = db.Address.FirstOrDefault(x => x.SinoszUser == a.SinoszUser && x.AddressType.AddressTypeName == "Otthoni").AddressText,
                //    GenusName = a.SinoszUser.Genus.GenusName ?? "",
                //    EducationName = a.SinoszUser.Education.EducationName ?? "",
                //    MaritalStatusName = a.SinoszUser.MaritalStatus.MaritalStatusName ?? "",
                //    EnterDate = (DateTime?)a.SinoszUser.EnterDate,// ?? (DateTime?)null,
                //    Remark = a.SinoszUser.Remark,
                //    HearingStatusName = a.SinoszUser.HearingStatus.HearingStatusName ?? "",
                //    InjuryTimeText = a.SinoszUser.InjuryTime.InjuryTimeText ?? "",
                //    PensionTypeName = a.SinoszUser.PensionType.PensionTypeName ?? "",
                //    DecreeNumber = a.SinoszUser.DecreeNumber,
                //    NationText = a.SinoszUser.Nation.NationText ?? "",
                //    Sum = null == b ? 0 : b.Sum,//(float?)b.Sum ?? (float?)null,
                //    Email = db.Address.FirstOrDefault(x => x.SinoszUser == a.SinoszUser && x.AddressType.AddressTypeName == "E-mail").AddressText,
                //    //bizonylatadatok
                //    Id = a.Id,
                //    AccDocumentDate = (DateTime?)a.DocumentDate,// ?? (DateTime?)null,
                //    AccAccountingTypeName = a.AccountingType.AccountingTypeName,
                //    AccDocumnetNumber = a.DocumnetNumber,
                //    AccSum = a.Sum,
                //    AccAccountingStatusName = a.AccountingStatus.AccountingStatusName
                //})
                ////.OrderByDescending(x => x.AccDocumentDate)
                //.AsEnumerable();
            ...
            if (null == Output)
            {
                ...
                result.rows = (from r in rs
                               ...
                               // r.Id.ToString()
                                ...
                                //,r.Sum.ToString()
                                //,r.Email
                                //,r.AccDocumentDate.ToString()
                                //,r.AccAccountingTypeName
                                //,r.AccDocumnetNumber
                                //,r.AccSum.ToString()
                                //,r.AccAccountingStatusName
                               {
                                   id = r.a.Id.ToString(),
                                   cell = new string[] 
                               { 
            ...
            else if ("XLS" == Output || "PDF" == Output)
            {
                ...
                if ("XLS" == Output)
                {
                    ...
                    return new ExcelResult(Headers, ColunmTypes, lstgrid, "TagokFizetései.xlsx", "TagokFizetései");
                }
                else if ("PDF" == Output)
                {
        ...
        /// <summary>
        /// A nyomtatási view-t reszeli
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual ActionResult PrintPDF(GridSettings grid)
        {
            using (log4net.ThreadContext.Stacks["NDC"].Push("Sinosz/PrintPDF"))
            {
                log.Info("begin");
                log.Info("end");

                return new ActionAsPdf("ListAccountingDocumentsForList", 
                            new { _search = grid.IsSearch,
                                    page = grid.PageIndex,
                                    rows = grid.PageIndex,
                                    sidx = grid.SortColumn,
                                    sord = grid.SortOrder,
                                    filters = JsonConvert.SerializeObject(grid.Where),
                                    Output = "PDF" }) { FileName = "TagokFizetései.pdf" };
            }
        }
...
#region PrintContract
        ...
        /// <summary>
        /// A kommunikációs szerződés nyomtatása
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual ActionResult PrintCommContractPDF(Guid suserId)
        {
            using (log4net.ThreadContext.Stacks["NDC"].Push("Sinosz/PrintCommContractPDF"))
            {
                log.Info("begin");
                log.Info("end");
                return new ActionAsPdf("PrintCommContract",
                            new
                            {
                                suserId = suserId,
                                Output = "PDF"                                
                            ...
                            { 
                            FileName = "Szolgáltatási szerződés természetes személy felhasználónak.pdf",
                            PageMargins = { Left = 25, Right = 25, Top = 15, Bottom = 20 }, // it's in millimeters
        ...
        /// <summary>
        /// A kommunikációs szerződés adatforrása
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual ActionResult PrintCommContract(Guid suserId)
        {
            using (log4net.ThreadContext.Stacks["NDC"].Push("Sinosz/PrintCommContract"))
            {
                log.Info("begin");
                var su = db.SinoszUser.Find(suserId);
                var emailaddr = db.Address.FirstOrDefault(y => y.SinoszUser.Id == suserId && y.AddressType.AddressTypeName == "E-mail");
                var teladdr = db.Address.FirstOrDefault(y => y.SinoszUser.Id == suserId && y.AddressType.AddressTypeName == "Mobil");
                PrintContractViewModel pcvm = new PrintContractViewModel()
                {
                    SinoszUserName = su.SinoszUserName,
                    BirthName = su.BirthName,
                    MothersName = su.MothersName,
                    BirthDatas = string.Format("{0}, {1}", su.BirthPlace, su.BirthDate.ToString().Substring(0,10)),
                    HomeAddress = string.Format("{0}, {1}", (null == su.Postcode ? "" : string.Format("{0} - {1}", su.Postcode.Code, su.Postcode.City)), su.HomeAddress),
                    SinoszId = su.SinoszId,
                    Email = null == emailaddr ? "" : emailaddr.AddressText,
                    Telephone = null == teladdr ? "" : teladdr.AddressText,
                    DateNow = string.Format("{0}", DateTime.Now.ToString().Substring(0, 10))
                };

                log.Info("end");
                return View("PrintCommContract", "_PrintContract", pcvm);
            }
        }

        /// <summary>
        /// Az eszközkölcsönzési szerződés nyomtatása
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual ActionResult PrintDevContractPDF(Guid suserId)
        {
            using (log4net.ThreadContext.Stacks["NDC"].Push("Sinosz/PrintDevContractPDF"))
            {
                log.Info("begin");
                log.Info("end");
                return new ActionAsPdf("PrintDevContract",
                            new
                            {
                                suserId = suserId,
                                Output = "PDF"
                            ...
                            { 
                                FileName = "Készülék kölcsönzésére és használatára vonatkozó szerződés természetes személy felhasználók számára.pdf",
                                PageMargins = { Left = 25, Right = 25, Top = 15, Bottom = 20 }, // it's in millimeters
                                PageOrientation = Rotativa.Options.Orientation.Portrait,
                                PageSize = Rotativa.Options.Size.A4
                            };
        ...
        /// <summary>
        /// Az eszközkölcsönzési szerződés adatforrása
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public virtual ActionResult PrintDevContract(Guid suserId)
        {
            using (log4net.ThreadContext.Stacks["NDC"].Push("Sinosz/PrintDevContract"))
            {
                log.Info("begin");
                var su = db.SinoszUser.Find(suserId);
                var u = db.Users.FirstOrDefault(x => x.SinoszUser.Id == suserId);
                var dev = db.DeviceLog.FirstOrDefault(x => x.KontaktUser.Id == u.KontaktUser.Id);//mindig az első
                var emailaddr = db.Address.FirstOrDefault(y => y.SinoszUser.Id == suserId && y.AddressType.AddressTypeName == "E-mail");
                var teladdr = db.Address.FirstOrDefault(y => y.SinoszUser.Id == suserId && y.AddressType.AddressTypeName == "Mobil");
                PrintContractViewModel pcvm = new PrintContractViewModel()
                {
                    ...
                    Telephone = null == teladdr ? "" : teladdr.AddressText,
                    DateNow = string.Format("{0}", DateTime.Now.ToString().Substring(0, 10)),
                    DeviceId = null == dev ? "NINCS ESZKÖZ AZ ÜGYFÉLHEZ RENDELVE!" : null == dev.Devices ? "NEM TALÁLOM A HOZZÁRENDELT ESZKÖZT!" : dev.Devices.DeviceId
                };

                log.Info("end");
                return View("PrintDevContract", "_PrintContract", pcvm);
            }
        }
